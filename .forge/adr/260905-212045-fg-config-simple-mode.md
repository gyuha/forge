...
- **(개정 2026-09-06) `simple`을 켜면 `fg-adversarial-review`(ADR-0018)를 걸 수 없다 — 회고 skip 외의 두 번째 대가다.** 적대적 리뷰는 **활성 슬롯만** 대상인데(봉인 작업의 unpark 경로 없음), simple은 UAT 통과 즉시 같은 턴에 봉인해 그 슬롯을 비운다 — 즉 리뷰를 *요청할 기회가 오기 전에* 작업이 `done/`으로 넘어간다. 결정 1을 내릴 때 이 상호작용은 검토되지 않았고, task #137(문서 작업)의 적대적 리뷰가 역설적으로 이를 발견했다. 봉인 전 리뷰를 관행으로 쓰는 팀은 `simple`을 끄는 것이 맞다. 세 번째 대가도 함께 드러났다 — UAT 판정 자체가 틀린 채 봉인되면 `status: done`을 되돌리는 스킬이 forge에 없다(`fg-drop`도 `done/` 제외). simple이 없앤 회고·봉인 트리거 2회는 마찰이자 **오판을 잡을 재고의 관문**이었다. 사용자 문서는 `docs/config-modes.md`(ko/en)가 "대가 셋"으로 서술한다.
