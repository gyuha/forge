...
## 개정 (2026-09-02) — 논거 문장의 축소 (결론 불변)

개정(2026-08-20)의 논거 *"forge는 사용자 프로젝트에 `.gitignore`를 쓰지 않는다"*는 fg-showme의 자체-ignore 도입(ADR `260719-224442` 개정 2026-09-02 — `.forge/showme/.gitignore`에 `*` 한 줄)으로 문자 그대로는 거짓이 됐다. 정확한 형태는 **"forge는 사용자의 `.gitignore`를 편집하지 않는다 — 자기 상태 디렉터리 안의 자체-ignore는 예외"**다. **이 ADR의 결론은 불변이다**: gitignore 규칙은 어떤 형태든 "그 규칙이 있어야 안전"인 반면 리포 밖 경로는 커밋 경로 자체가 없으므로, 취약점 리포트는 여전히 리포 밖에 산다 — 자체-ignore가 생겼다고 감사 산출물을 리포 안으로 되돌리지 말 것.
