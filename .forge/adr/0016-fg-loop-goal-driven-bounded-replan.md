...
## 개정 2026-09-08 — 재개와 차단 해제를 구분

재호출을 해결 신호로 간주해 `wall: none`으로 초기화하던 규칙을 폐기한다. 범위 밖 C5 결함이 그대로인데 재개 → 동일 검사 → 동일 fork 안내가 반복된 사례가 근거다. stateless 재개는 영속 계약에서 판단을 복원한다는 뜻이지, 기존 차단 근거를 버린다는 뜻이 아니다.

재개 사전점검은 기존 승인과 관련 증거를 먼저 읽는다. 미해결·무변경이면 wall과 원장을 유지하고 같은 검사를 실행하지 않는다. 명시적인 재검사 요청은 관련 진단만 허용하며 수정 범위 확대가 아니다. 해결된 결정은 기존 계약 절에 기록하고 같은 턴에서 이어간다. fg-status와 fg-next도 미해결 상태에서는 재호출 명령 대신 필요한 결정·증거를 다음 행동으로 삼는다.

실패 관측 횟수와 수정 시도는 구분한다. 실제 재검사는 원장을 갱신하지만 생략한 검사는 횟수를 늘리지 않으며, 시도 없는 재검사를 두 번의 수정 실패라고 설명하지 않는다. 누적 라운드·지출은 재개로 초기화하지 않는다. `/goal`은 호스트에 따른 실행 지속 대안일 뿐 범위 승인이나 차단 해제 수단이 아니다. 새 최상위 상태 필드는 추가하지 않는다.

재검사로 `×2`가 된 뒤 명시적 범위·접근법 결정으로 새로운 수정이 가능해지면, 원장에 결정과 접근법을 기록하고 한 번의 제한된 시도를 허용한다. 그 결정의 사용은 `tried:` 시도와 함께 남겨 재사용하지 않는다. 증거가 그대로면 다시 no-progress로 멈춘다. 승인 후에도 첫 수정조차 불가능한 교착과 승인 재사용 무한 반복을 함께 막는다.

이 개정은 아래 과거 개정의 무조건 wall 초기화·매 재개 `/goal` 안내보다 우선한다. 검증 봉인 게이트와 범위·안전 제한은 유지한다. 반복 사례의 행동 회귀 입력과 판정 기준은 `skills/fg-loop/tests/resume-scenarios.md`에 둔다.
