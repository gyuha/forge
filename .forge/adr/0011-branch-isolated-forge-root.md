...
## 개정 (2026-07-17) — fg-merge 대화형 git-merge 모드

위 "통합은 git merge 뒤 fg-merge가"·"forge는 git을 실행하지 않는다"는 **코어 결정론 스크립트(`forge-merge.sh`/`.js`)·CI 경로에는 여전히 유효**하다(git-free 불변). 다만 **대화형 fg-merge 스킬은 인자를 주면(`fg-merge <branch>`) 편의를 위해 `git merge`를 먼저 실행**하도록 경계가 완화됐다 — 인자 없는 `fg-merge`는 종전대로 통합만 한다. 상세·트레이드오프(충돌 정지·스마트 라우팅·미커밋 리마인드·기본브랜치 전제)는 ADR `260717-10a` 참조.
