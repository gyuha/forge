...
이 4단계에 속하지 않는 12개 스킬. 루프 모양을 바꾸지 않고 보조한다.
...
- **fg-status** (`skills/fg-status/SKILL.md`) — 읽기 전용 상태 리포터. `.forge/`를 조사해 현황+다음 단계("어디까지 했나")를 출력하고 아무것도 쓰지 않으며 다음 단계를 자동 실행하지 않는다.
- **fg-doctor** (`skills/fg-doctor/SKILL.md`) — 읽기 전용 무결성 health check. `.forge/` 상태 계약과 영속 문서/매니페스트 정합을 검사해 위반을 severity(error/warning/info)와 항목별 actionable 수정 안내와 함께 보고한다("상태가 건강한가"). 손으로 편집되는 Markdown이 조용히 깨지는 부류(고아 run.md·STATUS 필드 손상·slug 페어링 불일치·half-sealed done/·매니페스트 버전 3곳 drift·README 이중언어 어긋남·CLAUDE.md 스킬 목록 누락)를 잡는다. **아무것도 쓰지 않고 자동 수정·자동 실행을 하지 않으며**, 수정은 사람이 fg-quick(trivial)/fg-ask(non-trivial)로. forge의 `init.sh` health check 대응물(ADR-0019).
...
**상태를 읽는 스킬은 둘이며 관심사가 다르다(ADR-0019).** fg-status는 *"어디까지 했나"*(진행 상황 + 다음 단계)를, fg-doctor는 *"상태가 건강한가"*(무결성 위반)를 답한다. 둘 다 읽기 전용이지만 fg-status는 진행을 보고하고 fg-doctor는 손상을 검출한다 — 섞으면 단일 책임이 흐려지므로 의도적으로 분리했다. 다만 `.forge/` 상태 계약을 바꾸면 양쪽의 검사/보고를 함께 고려해야 한다.
...
위 휘발 상태 + 아래 영속 문서/매니페스트는 모두 **fg-doctor의 무결성 검사 대상**이다(읽기만, 수정 안 함 — ADR-0019).
...
- **`.forge/adr/NNNN-slug.md`** — 아키텍처 결정. 세 조건(되돌리기 어렵다/맥락 없이 의아하다/진짜 트레이드오프) 모두 충족 시에만. 현재 활성 ADR은 `0001`~`0019`(`.forge/adr/`), 은퇴 ADR은 `.forge/adr/retired/`.
...
- **fg-doctor의 그룹별 루트 적용** — fg-doctor는 FORGE-ROOT를 검사 그룹마다 다르게 적용한다(ADR-0019). (a) 휘발 상태 검사(그룹 A)는 **해석된 브랜치 루트만** 읽는다 — 휘발 상태는 브랜치-로컬이라 다른 브랜치 슬롯을 검사하면 틀린다. (b) 영속 문서 ADR 무결성 검사(그룹 B13)는 비-기본 브랜치에서 **FORGE-ROOT 읽기 오버레이**(최상위 + 브랜치 루트, 충돌 시 브랜치 우선)를 읽는다 — 브랜치 루트만 보면 브랜치 ADR이 최상위 ADR을 참조한 교차참조를 거짓으로 dangling 처리한다. (c) 리포 루트 검사(매니페스트·README·CLAUDE.md·`skills/`)는 브랜치 무관이다.
...
- **상태 무결성 점검** → `fg-doctor`(읽기 전용 health check, 수정은 fg-quick/fg-ask로).
