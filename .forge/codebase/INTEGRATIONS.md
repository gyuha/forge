...
`package.json`의 `release:check` → `node scripts/release-check.js`(bash 트윈은 `scripts/release-check.sh`). 네트워크·API 없이 리포 자신의 파일만 읽어 배포 전 7가지를 확인한다: 매니페스트 3개 존재, **버전 4곳 동기**(`.claude-plugin/plugin.json` · `marketplace.json`의 `metadata.version`·`plugins[0].version` · `.codex-plugin/plugin.json` — 현재 **0.8.6**), **Codex 매니페스트의 `skills`가 `"./skills/"`**, `hooks/hooks.json` 존재, `hosts/{claude,codex}/{interaction.md,execution.md,capabilities.json}` 6파일 완전성, 그리고 **신규 2건** — `core/HOST.md`의 표에서 **파싱해 도출한** capability 어휘로 두 `capabilities.json`이 평평한 불리언 객체이며 키가 정확히 그 8개인지, `docs/codex.md`·`docs/en/codex.md`가 존재하며 그 8개 키를 **모두 이름 짓는지**. 어휘를 스크립트에 하드코딩하지 않은 이유가 주석에 있다 — 사본을 두면 그게 바로 이 검사가 막으려는 드리프트다.
