...
├── skills/                  fg-* 스킬 19개 — 자동 탐색 (마크다운 28파일 3,072행)
├── hooks/                   배포 훅 1개 + polyglot 래퍼 + 래퍼 테스트 (3 파일)
├── scripts/                 결정론 bash/node 트윈 17 + 셸 테스트 15 = 32 파일
├── docs/                    사용자 문서(한국어) 6 + 랜딩 페이지 + 이미지 4 + 예제 1
├── .forge/                  forge 자신의 루프 상태 + 영속 문서 (dogfooding)
├── .claude/                 이 리포 개발용 프로젝트 자산 (배포 대상 아님)
├── CLAUDE.md                에이전트용 리포 지침 — 147행 / 36KB (사실상 스펙 문서)
├── README.md (190행) / README.ko.md (189행)   이중언어 쌍
├── CHANGELOG.md             590행 / 89KB — Keep a Changelog 약식
├── .gitignore               .forge/ 화이트리스트 규칙이 여기 있음 (24행)
...
**버전은 3곳을 반드시 동기 갱신**한다 — 현재 전부 `0.6.0`:
...
두 `description`은 역할이 다르다. `marketplace.json`의 `metadata.description`은 **루프만** 정의하는 한 줄 태그라인(루프 밖 유틸리티 제외), `plugins[].description`과 `plugin.json`의 `description`은 **전체 스킬 목록**을 담는 긴 설명(루프 밖 스킬 포함, "Nineteen fg-* skills"로 시작). 이 두 긴 설명은 각각 1만 자를 넘는 단일 문자열이다.
...
| `fg-status/` | fg-status | 119 | — | 루프 밖 (읽기 전용 리포터) |
...
| `fg-visual/` | fg-visual | 49 | `VISUAL.md`(312), `LICENSE`(21), `scripts/`(5파일) | 루프 밖 (시각 컴패니언) |

**19개 전부 디렉터리명 == frontmatter `name`**(일치 확인). 다만 **식별자는 frontmatter의 `name`이지 디렉터리명이 아니다** — 개명 시 둘을 함께 바꿔야 하고, `name:`이 아예 없으면 자동 탐색되지 않는다(`forge-doctor` B10이 error로 잡는다).
...
이 `scripts/`는 루트 `scripts/`(결정론 트윈)와 **완전히 별개**다 — 명명 규칙(`forge-*` 접두 없음)·언어(`.cjs`)·테스트 유무·계약이 모두 다르다. 세션 파일은 모든 브랜치에서 최상위 `.forge/visual/<세션>/`에 쓴다(휘발·gitignore — 브랜치 루트 아님). 벤더링 원본은 무변경 유지 방침이라, 텍스트 입력 이벤트도 서버의 `choice` 가드를 통과하도록 호출 측에서 `choice: "text:<field>"`를 함께 싣는 방식으로 우회했다.

### 3-B. 크기 감각 (참고)

가장 긴 것들: `fg-visual/VISUAL.md` 312행, `fg-statusline/SKILL.md` 217행, `fg-run/SKILL.md`·`fg-loop/SKILL.md` 각 180행, `fg-done/SKILL.md` 178행, `fg-eco/ECO.md` 156행, `fg-next/SKILL.md` 144행. 가장 짧은 것: `fg-run/RUN-ALL.md` 16행(progressive disclosure 분리 조각), `fg-tdd/SKILL.md` 47행. `skills/` 전체 마크다운(28파일) 합계 **3,072행**.
...
```
.forge/
├── config.json          [추적] 전역 예외 — 모든 브랜치에서 최상위. 현재 {"eco": true}
├── CONTEXT.md           [추적] 도메인 글로서리
├── adr/                 [추적] 43개 (retired/ 아직 없음)
├── retro/               [추적] 54개
...
│
├── plan.md              [휘발] 활성 슬롯 (현재 1건 점유)
├── run.md               [휘발] 활성 슬롯 (현재 있음)
├── STATUS.md            [휘발] 활성 슬롯 동반 마커 (현재 있음 — retro: pending)
├── backlog/             [휘발] 미실행 대기열 — 현재 0건
├── executed/            [휘발] 실행됨·미회고 park — 현재 0건
├── done/                [휘발] 봉인 아카이브 — 현재 114건, 각 dir = plan.md+run.md+STATUS.md
├── quick/LOG.md         [휘발] fg-quick 한 줄 로그 (현재 40행)
├── dropped/             [휘발] fg-drop 보관 — 현재 1건 (기본 브랜치에선 gitignore)
└── visual/              [휘발] fg-visual 세션 2건 + .last-port/.last-token — 전역 예외(항상 최상위)
```

현재 트리에 **없는** 휘발 파일(조건부 lazy 생성): `ask.md`(fg-ask 그릴링 표시 마커), `review.md`(적대적 리뷰 findings), `loop.md`(goal 계약).
...
| 회고 — 현행 | `YYMMDD-HHMMSS-slug.md` (5개) | `260730-233816-session-start-hook-hardening-fix.md` |
| 회고 — 레거시 | `YYYY-MM-DD-slug.md` (49개) | `2026-07-06-fg-done-seal-summary.md` |
| 봉인 디렉터리 — 현행 | `done/<YYMMDD-HHMMSS>-<slug>/` (같은 초·같은 slug 충돌 시 serial letter) — 17개 | `done/260731-154221-forge-merge-context-term-parser/` |
...
**시간ID 스킴 세 형식이 공존한다.** 순차 `NNNN`은 동결(더 이상 발급 안 함, 32개에서 멈춤), `YYMMDD-HH<letter>`는 과도기(4개), 현행은 `YYMMDD-HHMMSS`(7개)다. 근거: `.forge/adr/260716-13a-adr-time-based-id-scheme.md`, `.forge/adr/260719-161701-time-precise-naming.md`. 시계 기반이라 병렬 브랜치가 공유 카운터에서 충돌하지 않는 것이 요점이고, `forge-doctor` B14가 두 형식 모두의 ID 유일성을 검사한다.
...
│                                       data-l="ko" / data-l="en" span으로 병기 (각 77개, 균형)
├── skills.md                           128행 — 6열 스킬 카탈로그(19행) + 스킬별 상세
...
| 고치는 곳 | 함께 고쳐야 하는 곳 |
| --- | --- |
| 스킬 추가/개명/삭제 | `plugin.json` description · `marketplace.json` plugins[].description(+"Nineteen…" 개수 문구) · `CLAUDE.md` 스킬 목록 · `README.md` **+** `README.ko.md` · `docs/skills.md` |
...
- **스킬 개수**: 트리 19 · `docs/skills.md` 카탈로그 19행 · `marketplace.json` "Nineteen" — 일치. 단 `.claude/agents/manifest-doc-syncer.md`의 `description`만 **"18-스킬"**로 남아 있다(개발용 카드라 doctor B12 범위 밖). 트리가 정답.
...
- **`README.md` 190행 vs `README.ko.md` 189행** — 행수 자체는 번역 길이 차이로 정상 범위이고, doctor B13은 행수가 아니라 **스킬 행 개수**로 드리프트를 판정한다(현재 통과).
