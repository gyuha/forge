...
| `fg-next` | Orchestrator (outside the loop) | Derives the single next step via fg-status's state machine and, after one confirmation, runs it — one-shot; fg-status reports, fg-next acts | `.forge/*` (read-only itself) | none — delegates to the invoked skill | — (invokes the next skill) |
