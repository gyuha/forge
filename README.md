...
한 스킬이 끝나면 다음 스킬로 가는 길(무엇을 했고, 다음은 무엇이며, 어떻게 시작하는지)을 안내한다. 루프는 `fg-complete`가 작업을 봉인한 뒤, **새 작업**으로서만 `fg-ask`에서 다시 시작된다 — 같은 작업을 다시 실행하지 않는다.
...
- 활성 `.forge/`가 비어 있으면 = 진행 중 작업 없음. `fg-execute`는 빈 상태에서 실행하지 않는다(재실행 방지).
