...
> A loop-style workflow plugin built from thirteen `fg-`-prefixed Claude Code skills — four that form the loop, plus nine utilities outside it (`fg-map`, `fg-quick`, `fg-status`, `fg-next`, `fg-loop`, `fg-tdd`, `fg-eco`, `fg-merge`, `fg-cleanup`).
