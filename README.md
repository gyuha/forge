...
> A loop-style workflow plugin built from fifteen `fg-`-prefixed Claude Code skills — four that form the loop, plus eleven utilities outside it (`fg-map`, `fg-quick`, `fg-status`, `fg-next`, `fg-loop`, `fg-tdd`, `fg-eco`, `fg-merge`, `fg-cleanup`, `fg-statusline`, `fg-adversarial-review`).
