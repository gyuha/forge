...
> A loop-style workflow plugin built from fourteen `fg-`-prefixed Claude Code skills — four that form the loop, plus ten utilities outside it (`fg-map`, `fg-quick`, `fg-status`, `fg-next`, `fg-loop`, `fg-tdd`, `fg-eco`, `fg-merge`, `fg-cleanup`, `fg-statusline`).
