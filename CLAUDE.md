...
**문서 사이트를 검증할 때 반드시 볼 세 가지** — 라이트모드에서 페이지가 200으로 열리는 것만으로는 부족하다:
...
- **외부 `https://` 링크는 직접 확인한다.** VitePress의 dead-link 검사는 **내부 상대 링크만** 본다 — 그래서 `npm run docs:build` 통과는 외부 링크 유효성의 근거가 **되지 못한다**. 특히 `github.com/gyuha/forge/blob/main/.forge/adr/<파일>` 형태의 ADR 링크는 파일명을 한 글자만 틀려도 조용히 404가 되고 빌드는 초록으로 통과한다(실제로 `0011-branch-scoped-...`로 오기했다가 적대적 리뷰의 세 렌즈가 독립 수렴해서야 잡혔다 — task #137). DoD에 "빌드 통과"만 적지 말고 외부 링크 대상의 **실재**를 따로 확인할 것.
...
- **fg-run** — `.forge/plan.md`를 활성 호스트의 실행 어댑터로 실행(Claude Code는 Dynamic Workflow, Codex는 collaboration/subagent), 계획↔실제 차이를 `.forge/run.md`에 기록.
- **fg-learn** — 학습을 분류해 영속 문서로 승급, `.forge/retro/`에 회고 남김. 항상 대화형. 기계 확인 가능한 학습은 네 번째 목적지 **eval**(프로젝트 테스트 스위트의 영속 회귀 체크; 스위트 없는 리포는 fg-doctor 검사 같은 상응 착지점)로 승급한다 — ADR `260906-171420`.
...
- **상황별 출력 예시**: [skills/fg-next/OUTPUT-EXAMPLES.md](skills/fg-next/OUTPUT-EXAMPLES.md)는 스킬 작성자가 특정 스킬의 출력 개선을 검토할 때 선택적으로 읽는 참고 자료다. 대화 재개·사용자 수행 절차·긴 보고의 전후 예시를 한 벌로 유지한다. 모든 스킬이나 새 스킬에 읽기 지시를 일괄 추가하지 말 것. 특정 지점의 개선 필요가 확인되면 해당 예시의 적용을 검토하되, 기존 canonical 설명 문단·핸드오프·무인 주행 출력 억제를 유지한다.
...
- **문서 사이트 이중 언어 동기화**: 같은 규율이 문서 사이트에도 적용된다 — `docs/<name>.md`(한글, root locale)와 `docs/en/<name>.md`(영문)는 **8쌍의 번역 쌍**이며, 한쪽을 고치면 반드시 다른 쪽도 함께 고친다. README 1쌍이던 부담이 9쌍으로 늘어난 것이므로 **번역문은 절 구조를 1:1로 유지**해야 한다(`##` 헤딩의 개수·순서, 표의 행 수·열 수를 원문과 일치). 그래야 동기화 여부를 눈이 아니라 diff로 확인할 수 있다. 쌍이 빠지지 않았는지는 아래 한 줄로 확인한다:
...
- **조건부 분할은 문단 단위로 판정하고, 상호참조를 양방향으로 훑는다**: 스킬 본문 일부를 "조건 C일 때만 읽는" 동반 파일(`RUN-ALL.md`·`ALL-MODE.md`·`RECOVERY.md`·`INQUIRY.md` 계열)로 옮길 때 단위는 **헤딩이 아니라 문단** — *빈 줄로 구분된 최상위 블록*이며 **코드펜스는 1단위·목록은 최상위 항목별·blockquote 머리말은 1단위**다(`RECOVERY.md`는 `###` 0개인데 분기 3개를 굵은 문단으로 담아 헤딩 판정이 공허하게 통과한다). 검증은 **두 단계 체크리스트**이지 단일 기계 검사가 아니다 — ① 남은 문서와 옮긴 문서 **양쪽**의 상호참조(`§N`·"above"·절 이름)를 전부 열거하고 ② 가리키는 내용이 그 절에 **실재하는지** 확인한다. ①만 하면 절이 존재한다는 이유로 빈 포인터가 통과한다. 놓치면 유실이 아니라 **도달 가능성 상실**이라 크기·총량·링크 검사를 전부 통과한다(4연속 재발: 회고 `260911-160340`).
