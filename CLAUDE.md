...
1. **커밋** — 이 작업이 바꾼 파일만 커밋한다. 커밋 메시지에 `(Fixes #N)`을 포함해 GitHub의 커밋-이슈 자동 링크·자동 닫힘을 이용한다(예: `fix(...): ... (Fixes #N)`). 이것은 위 "배포 규칙"(CHANGELOG 갱신·3곳 버전 범프까지 포함하는 전체 릴리스 절차)과 **다르다** — 이슈 하나 봉인마다 버전을 올리지 않는, **커밋+push만의 가벼운 배포**다.
