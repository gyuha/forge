...
- **핸드오프**: 각 스킬은 끝에서 "방금 한 것 / 다음 단계 / 시작하는 법"을 **자연스러운 대화체**로 전한다(정해진 양식을 사무적으로 출력하지 않는다). **전환은 진술형** — "진행할까요?"로 다음 단계 진입을 *묻지 않고* 다음 스킬·트리거를 알리고 멈춘다. 체이닝(동의 시 다음 스킬 자동 호출)은 **fg-next 전담**이다. **예외는 fg-run 단일작업 종료뿐** — 분기가 많아 4지 명시 메뉴(회고 후 봉인까지[기본·divergence 무관, 회고 중 재그릴 권고 시 자동 봉인 중단] / 회고만 / 바로 종료=skip+봉인[저-divergence에서만] / 프롬프트로 나가기)를 제시한다. Run-all 배치 핸드오프도 진술형이다("어느 것부터?"는 fg-learn 소유 질문 — 중복 금지). 핸드오프에 "진행할까요?"를 재추가하지 말 것 — 정합·근거는 [ADR-0015](.forge/adr/0015-fg-run-handoff-menu-others-stated.md)(개정 2026-06-11).
