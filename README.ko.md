...
| `fg-visual` | 유틸리티 | 브라우저 시각 컴패니언(superpowers vendoring, MIT) — zero-dependency 로컬 서버가 에이전트가 push하는 HTML(목업·다이어그램·A/B 시각 비교)을 표시하고 당신의 답을 이벤트로 되읽음 — 선택형 화면의 필수 확정 버튼을 누르면 터미널 턴 없이 바로 당신을 깨우고, 탐색 클릭만으로는 깨우지 않음; fg-ask 그릴링 중 just-in-time 1회 제안, `fg-visual stop`으로 종료. 프레임은 리포 루트 `DESIGN.md`가 정의하는 Anthropic/Claude 디자인 시스템(크림 캔버스 + 코랄 강조 + serif 디스플레이)으로 렌더되므로, 에이전트가 미는 모든 화면이 그것을 상속한다 |
