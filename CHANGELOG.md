...
### Added
- **Pi 기본 호스트 지원을 추가한다** — 기존 22개 `fg-*` 스킬과 `.forge/` 상태 계약을 그대로 로드하며, 별도 확장 없이 대화형 계획·순차 실행·검증·봉인을 지원한다. `hosts/pi/` 어댑터, 영·한 사용 가이드, VitePress 내비게이션, 실제 Pi 0.85.1 로더 기반 검증을 함께 추가했다.
- Codex가 프로젝트 로컬 `issue-triage` 스킬을 탐색할 수 있도록 링크를 추가했다.

### Changed
- 스킬 작성자가 사용자 대면 출력 형식을 선택적으로 문서화할 수 있도록 작성 가이드에 출력 예시 규약을 추가했다.
- 릴리스 게이트가 새 호스트 어댑터와 양언어 지원 문서를 함께 검증하도록 확장했다.

## [0.9.2] - 2026-09-11

### Added
- **`fg-doctor`에 자기참조 검사(B21)를 신설한다** — `skills/*/SKILL.md`에서 `§N` 참조가 §N 절 **안에** 있는 경우를 잡는다. 조건부 분할이 내용을 옮긴 뒤 남긴 빈 포인터의 전형이며, 대상 절은 여전히 **존재**하므로 크기·총량·링크·frontmatter 검사를 전부 통과한다. `.sh`/`.js` 트윈 양쪽에 구현하고 behavior 테스트 3케이스(양성 1 + **음성 2**)로 보증한다. **부분 가드임을 코드 주석과 문서에 명시** — 절을 건너뛰는 오참조는 의미 판단이라 잡지 못하며 그쪽은 `CLAUDE.md`의 2단계 체크리스트가 맡는다.
  - `.sh` 구현이 `awk`를 쓰지 않는 이유도 주석에 남겼다: BSD awk(macOS 기본)에 3인자 `match()`가 없어 awk판이 **모든 입력에서 조용히 0건**을 반환했고, 리포가 실제로 깨끗했기에 정상과 구별되지 않았다. fixture 음성 케이스가 그것을 드러냈다.

### Changed
- **`fg-loop`의 조건부 분할 경계를 "언제 쓰이나" 기준으로 재절단한다.** `## 1. Initial inquiry`가 *신규 goal 전용* 내용과 *주행 시점 규칙*을 함께 담고 있었던 탓에, 분할 시 후자(원장 `×N` 의미론·`waiting` 회계·Reflexion 기록 규칙·`/goal` 드라이브 진입)가 재개 경로에서 도달 불가가 됐다. 네 블록을 `SKILL.md` §2/§3/§4의 의미상 제자리로 되돌려 **재개 경로가 `SKILL.md` 단독으로 완결**되고, `INQUIRY.md`는 처음으로 전부 신규 goal 전용이 된다.
- **`§N` 상호참조 정합을 복구한다** — 이동을 따라가지 않은 참조 15곳을 재지정했다(자기참조 4→1→0 포함). `§1`이 스텁이 된 이상 내용이 사는 곳을 가리켜야 하므로, 질의 시점 선언 항목은 "`INQUIRY.md`에서 선언됨"이라는 출처 표기로 바꿨다.
- **`CLAUDE.md`의 조건부 분할 규율을 재설계한다** — 단위를 "헤딩이 아니라 문단"으로 **정의**하고(빈 줄 구분 최상위 블록 · 코드펜스 1단위 · 목록 최상위 항목별 · blockquote 머리말 1단위), "판단 대신 기계적 테스트"라는 거짓 주장을 **두 단계 체크리스트**(①양방향 참조 열거 ②내용 실재 확인, ②는 판단)로 교체했다.
- 문서 사이트: `SITE_URL` 기반 sitemap 호스트명과 소셜 메타(`transformHead`)를 구성하고 `og-image.png`를 추가한다.

### Fixed
- **무인 주행의 안전 구멍** — 주행이 실패 체크에 `evidence: external`을 **스스로 붙이는 것을 금지**하는 규칙이 `SKILL.md`에 없었다. 그 표식이 붙으면 체크는 실패가 아니고 `replan-round`를 쓰지 않으며 fix-forward 대상도 벽도 아니므로, cap 압박을 받는 주행이 **실패를 무기한 대기로 바꾸는 최소 저항 경로**가 열려 있었다. 금지 문장을 추가했다.
- **`regressed:` 카운터의 규칙이 둘이었다** — 한정(fix-forward일 때만)과 무한정 술어가 연속 문단에 공존해, 무한정 쪽을 따르면 평범한 다중 태스크 간섭에서 tension 벽이 오발한다. 한정 술어로 단일화했다.
- 스텁의 거짓 주장 둘(`INQUIRY.md`가 check-health probe를 소유한다 · "resume preflight는 §2가 소유한다"), 자기 절을 건너뛰라는 자기모순 문장, `INQUIRY.md` 머리말의 "바로 주행으로", dangling 단일-정의 포인터, 콜론↔blockquote 분리, `CLAUDE.md`의 해결 불가 `[[ ]]` 링크.

## [0.9.1] - 2026-09-11

### Changed
- **가장 무거운 스킬 셋의 드문 분기를 조건부 로드 동반 파일로 분리한다.** 기존 `RUN-ALL.md` 패턴을 그대로 적용해, 각 분기의 **진입 조건은 `SKILL.md`에 남기고** 상세는 그 조건이 실제로 성립할 때만 읽는 형제 파일로 옮겼다. **동작은 불변** — 문장을 옮긴 것이지 계약을 바꾼 것이 아니다.
  - `fg-next` 26,253 → 14,576자 (−44%) — `ALL-MODE.md`, `all` 인자를 줄 때만 로드
  - `fg-run` 45,614 → 40,985자 (−10%) — `RECOVERY.md`, 실행중 재진입·파킹 실패 회수·조기 종료일 때만 로드
  - `fg-loop` 62,654 → 45,568자 (−27%) — `INQUIRY.md`, `loop.md`가 없을 때(새 goal 최초 1회)만 로드
  - 공통 경로 합계 **134,521 → 101,129자 (−24.8%)**
- 분할이 끊는 **산문 절 참조**(`§2`, "Autonomy contract above")는 Markdown 링크 검사로 잡히지 않는다. 계약 문구를 고쳐 쓰는 대신, 각 동반 파일 머리말에 `§N`·`step N`·"above"가 `SKILL.md`의 절 번호를 가리키며 그 번호는 분할로 바뀌지 않았음을 명시했다.
...
### Added
- `fg-run` 실행 위임 직전에 `running.md` 마커를 기록하고 호스트별 회수 가능한 실행 핸들·세션·시각을 보존해, 세션 재진입이 같은 계획을 중복 실행하지 않도록 한다.
- 봉인·SessionStart·doctor의 마커 소유권, 세션 구분, JSON 경계, epoch 파싱, 파일 교체 경쟁을 검증하는 셸/Node 회귀 픽스처를 추가한다.

### Changed
- Claude와 Codex의 실행 수집·취소 의미를 호스트 어댑터로 분리하고, `fg-run`은 실행 중·완료·회수 불가 결과를 순서대로 판정한다.
- `fg-ask`·`fg-learn`·`fg-done`·`fg-drop`·`fg-next`·`fg-loop`와 한·영 상태 계약이 실행 중 마커의 전체 생명주기를 일관되게 처리한다.

### Fixed
- 봉인 뒤 고아 `running.md`가 남거나, 중단 시 마커만 지워져 다음 진입이 작업을 처음부터 다시 실행할 수 있던 경로를 닫는다.
- 무인 주행이 회수 불가능한 실행을 새 작업으로 재개하지 않고 사람 확인이 필요한 벽에서 멈추도록 한다.
- SessionStart 훅의 세션 판정·JSON 파싱과 doctor의 선행 0·큰 epoch 처리를 셸/Node 트윈에서 일치시킨다.
...
### Fixed
- 미해결 `fg-loop` 벽을 재호출만으로 초기화해 동일 검사와 `fork` 안내를 반복하던 흐름을 막는다.
- 기존 승인·관련 증거·명시적 재검사를 구분하고, 실패 관측과 실제 수정 시도를 별도로 누적한다.
- `fg-status`와 `fg-next`가 미해결 goal loop에서 단순 재개 대신 필요한 결정이나 증거를 안내한다.

### Added
- 재개·범위 승인·재검사·예산 소진·대기 상태를 다루는 행동 회귀 시나리오를 추가한다.

### Changed
- `/goal` 안내를 실행 가능한 주행에서만 사용하는 호스트별 폴백으로 정리한다.

## [0.8.7] - 2026-09-08

### Fixed
- 봉인 아카이브 준비 실패 시 원본 기록을 보존하고 파일 작업 오류를 exit 6으로 반환한다. 반봉인 복구에도 검증·회고 게이트를 적용한다.
- 여러 단계의 브랜치 경로를 자동 통합 대상으로 재귀 탐색한다.
- fg-map의 Claude 전용·직렬 실행 금지 잔여 지시를 호스트 어댑터 계약과 일치시킨다.
- Windows 훅 래퍼가 하위 훅의 종료 코드를 보존해 Stop의 exit 2를 전달한다.

### Added
- 봉인 실패·복구 게이트 및 깊은 브랜치 탐색 회귀 테스트와 shell/Node parity 사례.
- 실제 cmd.exe의 종료 코드 전달을 검증하는 Windows CI 작업.

### Changed
- 코드베이스 지도와 README·상태 계약 문서를 갱신한다.
...
### Fixed
- **`fg-help`의 frontmatter `description` 길이** — 768 → 555 코드포인트(상한 600). `description`은 이중 용도라 — `/fg` 메뉴 표시와 자동 호출 트리거가 **같은 문자 상한**을 공유한다 — 길어지면 메뉴 가독성과 트리거 정확도가 함께 나빠진다(ADR `260716-22a`). 뺀 것은 "스크립트 트윈이 없는 이유는 번역 때문"이라는 메타 설명 하나뿐이며, 같은 근거가 본문 「Why no script twin」 절과 `docs/skills.md` 쌍에 그대로 남아 있어 유실이 없다. 두 호출 형태·단일 소스 원칙·사용자 언어 렌더·트리거 7종은 전부 보존했다.
...
### Fixed
- **경로 해석 회귀 — 훅과 스킬 본문은 치환 메커니즘이 다르다.** `hooks/hooks.json`의 두 훅을 `${CLAUDE_PLUGIN_ROOT:-${PLUGIN_ROOT}}`로 역전했다. 셸 형식 훅 명령(`args` 없음)은 **치환되지 않고** 호스트가 변수를 훅 자식 프로세스 env에 넣으므로 안전하며, `PLUGIN_ROOT`(다른 도구도 export하는 일반적 이름)가 경로를 가로채는 것을 막는다. 텍스트 치환은 **스킬 본문** 메커니즘이라 그쪽 중첩 표기 64곳은 그대로 두는 것이 맞다 — 뒤집으면 에이전트가 미해석 셸 표현식을 읽는다. 이 비대칭을 `core/HOST.md`에 「두 메커니즘」 문단으로 못 박았다(*"한쪽을 다른 쪽에 맞춰 고치지 말 것"*).
- **`fg-help`가 스킬을 하나도 못 찾고 있었다.** Glob 인자가 `${...}` 패턴이면 리터럴로 매치되고 `../` 패턴이면 사용자의 작업 디렉터리 기준으로 해석된다 — **둘 다 조용히 0건**이다. Glob의 `path` 인자를 이 스킬의 상위 디렉터리로 고정하는 방식으로 바꿨다(패턴 `**/SKILL.md`). `docs/skills.md`·`docs/en/skills.md`의 표기도 함께 고쳤다.
- **훅 테스트의 동어반복 assertion을 교체했다.** `hooks.json`의 경로 표현을 grep하는 검사는 *"작성자가 방금 쓴 문자열이 파일에 있다"* 만 확인하므로, 누가 고칠 때는 실패하고 **틀렸을 때는 실패하지 않는다.** 적대적 `PLUGIN_ROOT` 하에서 명령 문자열을 실제로 실행해 확장 결과를 검증하는 4건으로 대체했다(25/0, 역전을 되돌리면 23/2로 정확히 실패).
- **`fg-doctor`의 두 fail-open을 닫았다.** ① 활성 슬롯에 `plan.md`+`run.md`는 있고 `STATUS.md`가 없는 상태를 아무 검사도 안 봤다(`A1`은 `plan.md` *부재*에만 발동, `executed/`에는 3파일을 요구하는 `A5`가 있는데 활성 슬롯에는 짝이 없었다) — `A1(b)` warning 신설. ② `B16` description 길이 검사가 YAML **접힘 블록 스칼라**에서 지시자 두 글자만 읽어 어떤 길이도 통과시켰다(상한 600에 가장 가까운 5개가 하필 사각지대였다).
- **`release-check`가 `capabilities.json`의 8키 계약을 실제로 검증한다.** 파일을 키 1개로 줄이거나 `not json`으로 파괴해도 `release:check`는 rc=0, `fg-doctor`는 error 0이었다(삭제할 때만 잡혔다). 정답 어휘는 하드코딩하지 않고 `core/HOST.md`의 표에서 **파싱**한다 — 두 번째 사본을 두면 그 사본이 곧 이 검사가 막으려는 드리프트가 된다.
- **`docs/codex.md` ↔ `capabilities.json` 동기 게이트를 추가하고, 그 게이트 자신의 fail-open 둘을 닫았다.** ① 지원표에 **행 자체가 없던** `spawn_role`·`plugin_root` 2개를 채웠다 — 문서가 스스로 주장한 *"같은 선언의 두 형태"* 가 문자 그대로 거짓이었다. ② 지원표 문서가 없으면 게이트가 조용히 통과하던 분기를 error로 바꿨다. ③ `release-check.yml`의 트리거에 `docs/codex.md` 경로를 추가했다 — 게이트가 **잡으라고 만든 드리프트에 정작 발동하지 못했다.**
- **호스트 어댑터가 `.forge/` 의미론을 갖고 있었다.** `hosts/claude/execution.md`·`hosts/codex/execution.md`에서 `.forge/run.md`·UAT 소유권 서술을 걷어내고 `core/EXECUTION.md`를 가리키게 했다 — ADR `260903-080713`이 *"어댑터가 `.forge/` 의미론을 조금이라도 갖기 시작하면 이 ADR이 무효"* 라고 못박은 자리다. 아울러 `hosts/<host>/…`를 지시하는 8곳에 상대경로 fallback을 줬다(`core/`가 이미 받던 대우).

### Changed
- **`PLAN-FORMAT.md`에 DoD 규칙 (e) — DoD는 산출물의 *동작*을 재고, 그 대리 지표를 재지 않는다.** 형제 (a)~(d)와 달리 **유일하게 통과하면서 결함을 숨기는** 형태라 별도 규칙으로 뽑았다. 실제 사례: 이번 릴리스의 한 완료 기준이 `../`라는 *문자열*이 타이핑됐는지만 세어 초록이었는데, 그 패턴은 Glob 도구로 **0개**를 매치했다 — 슬라이스의 목적 자체가 미달인데 모든 DoD가 통과했다.
- **ADR-0018 개정** — 적대적 리뷰가 생성한 fix-forward plan을 승인할 때 각 완료 기준을 위 (a)~(e)에 대고 한 번 훑는다. finding은 *무엇이 잘못됐나*를 말하지만 *고쳤음을 어떻게 증명하나*는 말하지 않고, 사람 그릴링(fg-ask)이라면 되묻는 그 질문이 리뷰→plan 경로에는 없다. 표본 2건이 모두 이 경로에서 나왔고, 두 번째에서 양상이 나빠졌다(못 충족해서 시끄럽게 → **통과하면서 숨음**).
- **`fg-loop`의 `budget-tokens` 계량기가 호스트에 결합돼 있다는 사실을 명시했다.** `forge-loop-spend`는 Claude Code의 트랜스크립트 파일을 읽는다. 동작 자체는 이미 fail-closed였으므로(루트가 없으면 exit 5 → `blocked-health` 벽) 스크립트는 고치지 않고, 무엇을 읽는지·다른 호스트에서 왜 정지하는지·나가는 길 둘(`budget-tokens: none` 또는 `--transcripts DIR`)을 적었다. Codex 트랜스크립트 위치는 아무도 관측하지 않았으므로 내장 경로를 만드는 것은 `core/HOST.md`의 *"관측한 것만 `true`"* 규율을 정면으로 어기는 발명이 된다.
- **릴리스 게이트가 CI에 배선됐다.** `release:check`는 사람이 배포 절차 중 손으로 돌리는 것이 유일한 발동 경로였다. `docs.yml`에 스텝으로 넣지 않고 **별도 워크플로우**(`release-check.yml`)로 뺀 이유는 그쪽 트리거가 `docs/**`·`package.json`뿐이라 정작 지켜야 할 `.codex-plugin/`·`core/`·`hosts/` 변경에 발동하지 않기 때문이다. 두 트윈과 패리티 테스트를 함께 돌린다.
- `.forge/codebase/` 지도 7문서를 증분 갱신했다(Codex 어댑터 경계·`.codex-plugin` 매니페스트·`release-check` 트윈·`fg-showme` 세션 격리 반영). 매퍼 보고 8건을 재측정해 3건을 정정했다.
...
### Added
- **`fg-next all`·`fg-loop`의 태스크당 커밋 (옵트인, 기본 off).** `.forge/config.json`의 `driveCommit`(**엄격 불리언**)을 켜면 태스크를 봉인할 때마다 그 태스크의 변경을 **로컬 커밋**해 태스크 단위 롤백 지점을 남긴다. `driveCommitMessage`로 메시지 형식을 바꿀 수 있고 치환자는 `{title}`·`{slug}`·`{task}` 셋뿐이다(미지 치환자는 기본 형식으로 폴백 + 경고). 단일 정의는 `skills/fg-next/DRIVE.md` Part 3, 근거와 기각 대안 전부는 ADR `260901-213128`.
- **경계 여섯 개를 함께 못 박았다.** ① **커밋만, push는 안 함** — 롤백은 로컬 커밋에서 완결되고 push는 되돌림 가능성만 뺀다(플러그인에 `git push` 지시는 여전히 0건). ② **스테이징을 추론하지 않음** — 주행 진입 시 트리가 깨끗해야 커밋하고(그때 `git add -A`가 곧 "이 태스크가 바꾼 것 전부"), 더러우면 그 주행은 커밋 없이 진행하며 한 줄로 알린다. ③ **커밋할 것 없음 = 상태**(조용히 계속) / **거부 = 벽**. ④ **열 번째 벽을 만들지 않음** — 거부는 기존 `fork (commit rejected — <사유>)`에 접어 파급 4곳(`wall:` enum·"all nine walls" 문구·fg-status 진단·fg-next 정지 집합)을 0으로 줄였다. ⑤ **무인 경로만** — 명시적 `/fg-done`은 사람이 있고, `fg-done all`은 스테이징 정의가 성립하지 않으며, `fg-run` Run-all은 봉인하지 않는다. ⑥ **조작은 스킬 계층에만** — `forge-done.sh`/`.js`는 계속 git-free다.
- **지시문 규율 2건 승급** (회고 `260901-223116`에서 사람 승인). `CLAUDE.md` 「스킬 편집 규약」에 **"분기를 쓸 땐 판정을 쓴다"** — 계약에 둘 이상의 결과를 적으면 무엇으로 구분하는지 같은 문단에 적는다. `PLAN-FORMAT.md`의 runnable-DoD 규칙에 **(d)** — DoD는 산출물을 검사하지 산출물이 *기대는 원시 동작*은 검사하지 않으므로, 계약이 외부 도구에 의존하면 그 동작을 한 번 재현한다.

### Fixed
- **작성한 계약 자체의 판별 결함을 실행 중에 잡았다.** 주행 커밋 규율이 "커밋할 것 없음 / 훅 거부" 두 결과를 서술했는데, 실측해 보니 **둘 다 `git commit` exit code가 `1`**이었다 — exit code로 구분이 불가능하고 메시지 파싱은 취약하다. 거부를 "없음"으로 오분류하면 거부된 커밋을 지나쳐 봉인하는데, 봉인은 활성 슬롯을 비우므로 롤백 지점도 forge 상태도 사라진다. **순서로 결정론화**했다: `git add -A` 후 스테이징이 없으면 `commit`을 아예 실행하지 않고, 스테이징이 있을 때만 커밋해 그때의 실패는 전부 거부다. 이 발견이 위 `PLAN-FORMAT (d)`를 낳았다.
- **`scripts/forge-done.sh`의 주석 드리프트.** JUDGMENT 목록이 *"issue-linked commit … stays in fg-done's prose"*라고 적었으나 그 산문은 존재한 적이 없다(`skills/fg-done/SKILL.md`에 `issue` 0건). 각 항목의 실제 거처를 가리키게 고치고 **과거 주석이 거짓이었다는 사실을 명기**했다 — 조용히 고치면 다음 독자가 같은 오해를 반복한다.

### Docs
- `README.md`·`README.ko.md`(L3 무인 절 + `fg-loop` 행)와 `docs/skills.md`·`docs/en/skills.md`(fg-loop 절)에 옵트인 주행 커밋을 이중언어로 반영. `CLAUDE.md`는 카탈로그 2곳과 함께 **`config.json` 키 목록 4개**(`tdd`·`eco`·`driveCommit`·`driveCommitMessage`)를 명시하고, 앞의 둘만 전용 토글 스킬이 있는 **의도된 비대칭**을 적었다.
