...
#
# Each case builds a fake CLAUDE config dir (orig.sh + the real fragment), pipes
# a session JSON, and checks the wrapper's composed output.
#
...
# Build a fake CLAUDE config dir: the real fragment + a stub original statusline.
# The stub proves the original is preserved AND that it receives stdin (it echoes
# a byte count of the JSON it read).
setup_home() {
  mkdir -p "$1"
...
# Run the wrapper with CLAUDE_CONFIG_DIR=$1 and JSON $2 on stdin, from cwd $3.
run_wrapper() { ( cd "$3" && CLAUDE_CONFIG_DIR="$1" printf '%s' "$2" | CLAUDE_CONFIG_DIR="$1" bash "$WRAPPER" ); }
