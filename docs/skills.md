...
| `fg-visual` | 시각 유틸리티(루프 밖) | 브라우저 시각 컴패니언 — obra/superpowers v6.1.1 Visual Companion vendoring(MIT 귀속). zero-dependency Node 서버가 에이전트가 push하는 HTML(목업·다이어그램·A/B 시각 비교)을 브라우저 탭에 실시간 표시하고, 사용자의 클릭·텍스트 입력을 이벤트(JSONL)로 받아 에이전트가 **답으로** 되읽음. fg-ask 그릴링 중 시각적 질문에서 just-in-time 1회 제안(거절 시 재제안 없음), 이 스킬은 단독 진입점(ADR `260719-224442`·`260730-224259`) | 에이전트가 push하는 HTML | 모든 브랜치 최상위 `.forge/visual/<세션>/`(휘발·전역 예외) + 클릭 이벤트(JSONL) | — (`fg-visual stop`으로 종료) |
