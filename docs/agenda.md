...
열린 질문은 **순차로 한 번에 하나씩** 해소하되, 기본은 **같은 대화에서 계속** 잇는다 — 하나를 정하면 바로 다음 질문이 뜨고, 세션은 당신이 멈추라고 하거나 열린 질문이 0이 될 때 끝난다(질문마다 재트리거하지 않는다):
...
- 해소기(resolver)는 기본이 fg-ask이며, 다른 것이 필요한 줄에만 그 줄에 이름을 적는다(예: fg-showme로 시각 비교). 티켓 유형 분류는 없다.
