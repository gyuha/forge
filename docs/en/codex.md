...
| `fg-showme` confirm-click wake (`event_wake`) | Unverified | Currently `false` — confirm on screen and then **send anything in the terminal** to resume; fg-showme states this once at session start. A candidate mechanism exists: `codex queue --thread <session_id> --message` (Codex ≥0.149.0) wakes an idle session (release notes: "wake idle sessions reliably"). Unlike Claude's `Monitor` (the agent pulls from a file) it is **push-shaped** — it needs a watcher that tails the events file and calls `codex queue`, plus the `session_id` from the `SessionStart` hook payload (hooks themselves cannot start a turn) — and the end-to-end observation inside fg-showme must be made from a Codex session; flip to `true` only after that observation |

This table is a **declaration**, not prose — the same claim lives in machine-readable form as the nine keys of `hosts/codex/capabilities.json`, and the two are always updated together. `npm run release:check` enforces that **all nine keys are named in this table** (the status wording itself is human-reviewed — the gate does not claim more than it checks). **A capability is `true` only when that host has been *observed* to provide it; unverified defaults to `false`** — every capability has a defined fallback (serial execution, a numbered list, a stated stop), and a fallback that runs is always cheaper than a tool call that does not exist. Flipping a `false` to `true` is an observation, not an assumption (`core/HOST.md`).
