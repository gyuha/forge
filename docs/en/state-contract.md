...
└── .forge/                    # every loop document lives here
    ...
    ├── running.md             # the in-flight marker (fg-run): written the moment execution is delegated (forge-running slug · workflow id · started epoch · session), deleted right after run.md lands — distinguishes "running in the background" from "plan only" so a re-entry never executes twice (ADR 260909-150614)
...
| `running.md` (the in-flight marker — written the moment execution is delegated, deleted right after `run.md`) | fg-run | fg-run (re-entry — collect · wait · confirm, never re-execute) · fg-status/fg-next (state machine 1b "in flight — wait") · the session-start hook (one line for a previous session's in-flight run) · fg-doctor (A10 orphan/stale marker) · fg-drop (removed with the active slot) |
