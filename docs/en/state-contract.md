...
### Seal failures and recovery

Sealing closes out STATUS in a temporary copy, publishes the complete archive, and then removes source files. Archive preparation failures preserve the source. File-operation errors return `SEAL_IO` and exit 6; a cleanup failure after publication can leave source residue alongside the archive, so inspect both before retrying. Half-sealed recovery must pass the verification and retro gates too.

Automatic branch integration recursively discovers names with multiple slashes, such as `feature/team/topic`. When several candidates exist, it requests a selection instead of choosing one arbitrarily.
...
| `running.md` (the in-flight marker — written immediately before launch with `workflow: pending`, updated with a collectable host execution handle when available (the workflow Task ID on Claude), deleted right after `run.md`) | fg-run | fg-run (re-entry — collect · wait · confirm, never re-execute) · fg-status/fg-next (state machine 1b "in flight — wait") · fg-ask/fg-learn/fg-done (do not refine, retro, or seal an in-flight slot; route to fg-run, while sealing deletes the marker as it clears the slot) · fg-next all/fg-loop (an uncollectable execution is a human-needed wall) · the session-start hook (same-session/other-session guidance) · fg-doctor (A10 orphan/stale marker) · fg-drop (cancel or warn, then remove it with the active slot) |
...
Re-invoking `fg-loop` is not authorization to clear a blocker. The saved `wall` remains until a resolving decision or relevant evidence is established. An unchanged blocker calls for the missing decision or evidence, not repeated checks. An already-given approval is recorded in the contract and execution continues in the same turn; `fg-status` and `fg-next` likewise do not route unresolved blockers to a bare resume command. An explicit recheck request does not expand scope or waive verification.
