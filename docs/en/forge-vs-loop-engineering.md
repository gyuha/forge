...
| --- | --- | --- |
| **Skills** — encode conventions and intent in SKILL.md, prevent intent debt | forge itself is 20 `fg-*` skills (the loop's four stages + 16 outside-the-loop utilities). Terms live in `CONTEXT.md`, decisions in ADRs, code context in the `.forge/codebase/` map — "docs are the loop's fuel, not its by-product" (pillar 2) *is* the defense against intent debt | README's two pillars, ADR-0001 |
