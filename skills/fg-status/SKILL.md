...
0. **`.forge/loop.md` exists** (a goal loop is in flight — fg-loop drives whole task loops itself) → the next step is resuming **fg-loop**, which re-derives per-task state internally and continues toward its stop condition (or reports the wall it halted at). Trigger: `/forge:fg-loop`. **Read `loop.md` and name which stop-condition checks are currently failing (and round N/cap) as part of stating this step — do NOT emit a bare "resume fg-loop" without the failing reason.** The `wall:` causes you may encounter are `no-progress` / `cap-exhausted` / `unverifiable-uat` / `fork` / `tension` / `safety` / `stalled-waiting` / `blocked-health` / `budget-exhausted`; a check sitting at `waiting ×N` with **no** `wall:` set is **not** a wall — the next step is still resuming fg-loop, but state that it is waiting on external evidence and that the human owes nothing (see the diagnostic layer above). This is load-bearing for every consumer of this state machine (fg-status reports it, **fg-next acts on it**): without the failing-check detail a halted loop looks byte-identical run after run, so the user just gets "resume fg-loop" repeatedly with no visible cause — the "keeps proposing the same task with no progress" failure mode. Still report the underlying task states below for visibility.
