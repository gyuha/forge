...
- **Active slot** — `.forge/plan.md`, `.forge/run.md`, `.forge/STATUS.md` (read its `status:`, `verified:`, and `retro:` fields), and `.forge/running.md` (the **in-flight marker** fg-run writes when it delegates execution and deletes once `run.md` lands — its presence without a `run.md` means the execution is still running or was never collected). This is the one task currently in flight.
...
1b. **Active `plan.md` and `running.md` exist, but no `run.md`** → the execution is **in flight** (fg-run delegated it to a background workflow/subagents and `run.md` has not been written). The next step is **to wait, not to run**: if this session launched it, the result arrives as a completion notification and fg-run continues on its own; re-entering fg-run is safe — it takes its in-flight resume (collect or wait, and if the execution belongs to another session it **asks** before rebuilding — see `../fg-run/SKILL.md` step 4a) — but it never re-executes. Say that plainly; a bare "next: fg-run (execute)" here is the "keeps telling me to run fg-run" bug (ADR `260909-150614`). Trigger: `/forge:fg-run` (collects/waits; asks before any rebuild).
