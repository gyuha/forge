...
1. **Active `run.md` exists** (a plan has run) — apply in this order (run → verify → learn → done, ADR-0009):
   - `STATUS.md` `verified: failed` → **the UAT ran and the result is broken** — fix it then re-run, or re-grill the plan. Routes to **fg-run** (fix-and-re-run; it also unparks a failed `executed/` task into the active slot — see fg-run's "Failed parked-task recovery") or fg-ask (re-grill), **not** fg-learn/fg-done. Trigger: "forge run" / `/forge:fg-run` (or "forge ask"). Highest priority — a failed result blocks both retro and seal.
