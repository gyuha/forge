...
**Host contract**: the wake watch — a confirmed click resuming the conversation with no terminal turn — depends on the host's `event_wake` capability. Read [../../core/HOST.md](../../core/HOST.md) and look the key up in `../../hosts/<host>/capabilities.json` **before** arming anything: it is `true` on Claude Code (the `Monitor` tool provides it) and `false` on Codex. On a `false` host the companion still works — every other behavior here is unchanged — but the user has to send a terminal turn after confirming, and you tell them so **once, at session start** (see Behavior 3). The `false` side is declared in `docs/codex.md`'s support table.

## What it does

- **`fg-showme` (no argument)** → start a companion session for this conversation: read [`VISUAL.md`](./VISUAL.md) (the full operating guide — when to use the browser vs the terminal, screen authoring, the event loop), start the server, **arm the wake watch if `event_wake` is `true`** (a persistent `Monitor` on the events file, so a confirmed click can wake you with no terminal turn), and share the keyed URL — on a `false` host, with the one-time fallback note in the same message. Then push screens as the conversation calls for them.
...
fg-showme (no arg)
   ...
   → event_wake true?  yes → arm the wake watch (Monitor, persistent)
   │                    no  → skip the watch; note the terminal-turn fallback once with the URL
...
3. **Arm the wake watch right after the server starts, and put a confirm button on every choice screen.** A persistent `Monitor` watches the events file for a confirmed selection or a submitted text event only — an exploratory click never matches, so browsing alone never wakes you (VISUAL.md's Starting a Session and Confirm button sections have the exact command and markup; do not restate them here). Whether the watch can be armed is the host's `event_wake` capability — look it up first (Host contract above) instead of discovering a missing `Monitor` at call time. When it is `false`, skip the watch and **say so once, in the message that hands over the URL**: on this host, press 확정 and then send anything in the terminal to resume. Once at session start — never repeated per screen (The Loop's per-screen line already tells them what to do for that screen).
...
- **The wake watch is a host capability (`event_wake`), never a hard dependency.** On a `false` host the companion runs exactly as before — no wake watch, the user presses 확정 (or submits an Ask input) and still sends a terminal turn to resume you — and that fallback is stated once at session start, not discovered by a failing tool call. Same graceful pattern as a missing `fg-map`/eco/tdd; the `false` side lives in `docs/codex.md`'s support table so a Codex user can know it before opening a session.
