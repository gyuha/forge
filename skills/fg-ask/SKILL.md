...
- **During grilling: apply Eco YAGNI lens when eco is on.** Read `eco` from the **top-level** `.forge/config.json` (global exemption — same path as `tdd`, never the branch root; treat as `false` if absent). If `true`, apply the Eco YAGNI lens from [`../fg-eco/ECO.md`](../fg-eco/ECO.md) to every slice discussion — no separate yes/no question, no mode announcement. eco=false or absent: no change to grilling.
