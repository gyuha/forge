...
- **When creating an ADR** (past the "Offer ADRs sparingly" gate above), mint its ID and fill its frontmatter per [ADR-FORMAT.md](./ADR-FORMAT.md): the **time-based ID** (`YYMMDD-HH` + the next free lowercase letter for that hour, scanning `adr/` incl. `retired/`), `author:` from `git config user.name` (if unset, ask once for a name), and `decided:` = today. Never renumber the grandfathered `NNNN` ADRs. Write the ADR body in the user's language, matching the project's existing ADRs.
