...
- **Volatile state.** On the default branch, the volatile loop files this skill writes (the backlog plan and active-slot state) are gitignored; the permanent docs it updates (CONTEXT.md, ADRs, retros) are git-tracked via the `.gitignore` whitelist, and a non-default branch root is tracked whole (see FORGE-ROOT.md / ADR-0011).
