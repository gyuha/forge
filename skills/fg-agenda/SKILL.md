...
**Forge root**: every `.forge/...` path below is **relative to the resolved forge root** — `.forge/` on the default branch, `.forge/branch/<branch>/` on any other branch. `agenda.md` is branch-aware like `loop.md`, **not** a global exemption. Resolve it per `${CLAUDE_PLUGIN_ROOT}/skills/fg-run/FORGE-ROOT.md` (skill-relative `../fg-run/FORGE-ROOT.md`) before reading or writing it (ADR-0011).
...
3. **No ticket-type taxonomy.** research / prototype / grilling / task are the names of *resolvers*, and forge already has the resolvers: conditional deep research (ADR-0006) · fg-visual · fg-ask · fg-quick. Adding a type field would be a second vocabulary for something already named. The default resolver is **fg-ask**; name another one only on the line that needs it.
