...
**Host contract**: the no-argument settings menu depends on the host's `structured_choice` capability. Read [../../core/HOST.md](../../core/HOST.md); when it is `false`, render the same choices as a numbered text list and accept a key name — the settings semantics do not change.
