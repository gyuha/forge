...
**Host contract**: read [../../core/HOST.md](../../core/HOST.md), [../../core/EXECUTION.md](../../core/EXECUTION.md), and the matching `../../hosts/<host>/execution.md` before delegating. Delegation depends on the host's `spawn_parallel` capability (and `spawn_role` for project agent cards); if the host is unknown or delegation is unavailable, execute serially while preserving every state transition and verification gate.
...
**1) Check the active slot.** If `.forge/plan.md` (the active slot) already exists, there is a task in progress — skip the backlog and go straight to step 4 below (only one task is active at a time). **But first look for `.forge/running.md`**: if it exists and `run.md` does **not**, the plan's execution is **in flight** (a background workflow or subagents were launched and their result was never collected) — take step 4a, never the normal start, because "plan.md with no run.md" is ambiguous on its own and re-running here is exactly the duplicate execution this guard exists to prevent. If any `.forge/executed/*/STATUS.md` carries `verified: failed` while the slot is occupied, note it in one line — its recovery waits until the slot frees (see step 5). Step 4 first separates an **interrupted UAT** (verification-only resume, no re-execution) from a genuine **re-run** before doing anything destructive.
...
**4a) In-flight resume — `running.md` present, `run.md` absent.** The **in-flight marker** `.forge/running.md` is written by §2 the moment execution is delegated and deleted by §4 right after `run.md` lands; if it is still there without a `run.md`, a previous turn (or session) launched the work and never collected it. This is **not** a normal start and **not** a re-run — nothing may be rebuilt silently. Read the marker (`workflow:` id · `started:` · `session:`) and branch on the **first matching case**:

- **Same session, execution still running** (the `workflow:`/task id is alive in this session) → **wait for it** (`TaskOutput` with `block: true`, repeating until it completes) and then continue at §3 exactly as if the turn had never ended.
- **Same session, execution finished** (the result is available but was never collected) → collect it, then continue at §3 (cross-verify → `run.md` → STATUS → UAT). Do not launch anything new.
- **Different session, or the execution cannot be found** (a workflow cannot be resumed across sessions) → say so in one line — "a previous session's execution of `<slug>` was in flight and cannot be resumed; its side effects may or may not have landed" — and **ask once** whether to rebuild and re-run. **Only on an explicit yes** delete `running.md` and enter §1 as a normal start; on no, leave everything in place and close with the handoff table (see "Early-exit handoffs" below). This is the one confirmation gate in this skill besides the duplicate-run warning, and for the same reason: the cost is a second execution.

Whichever case: **never delete the marker to make the ambiguity go away** — it is deleted only by §4 after `run.md`, or by the explicit rebuild above (fg-drop removes it with the active slot; fg-doctor A10 flags a stale or orphaned one). Rationale: ADR `260909-150614`.
...
- **Declined in-flight rebuild (step 4a)** — the marker and plan stay in place, so `Just did` = an unresumable in-flight execution was found and left untouched, `Next step` = re-enter fg-run when ready to rebuild (it will ask the same confirmation), `Alternative` = discard the task with fg-drop. Never name fg-learn or sealing — nothing has run to completion.
...
Active slot (.forge/plan.md) present?
   ...
   │            │           │ yes ──▶ same session & alive ──▶ wait (TaskOutput) ──▶ §3 onward
   │            │                     same session & finished ──▶ collect ──▶ §3 onward
   │            │                     other session / not found ──▶ ask once ──▶ yes: delete marker, normal start │ no: stop (handoff table)
...
**The moment execution is delegated, write the in-flight marker `.forge/running.md`** (the active slot, next to plan.md):

```
<!-- forge-running: {slug} -->
workflow: {the workflow run id, or `agents` for direct background subagents}
started: {epoch seconds}
session: {this session's id}
```

It exists because a delegated execution **always** runs in the background — the tool returns at once and completion arrives as a notification — so until `run.md` is written the state on disk is "plan.md only", indistinguishable from a promoted-but-unrun plan; every reporter would then say "next: fg-run" and a re-entry would rebuild the workflow (ADR `260909-150614`). The marker is what step 4a, fg-status's state machine, the session-start hook, fg-doctor (A10) and fg-drop read instead.

**Then do not end the turn — wait for the execution here.** Block on the launched run (`TaskOutput` with `block: true`; the wait caps at ten minutes per call, so repeat it until the run reports completion) and continue straight into §3 in the same turn. The marker is the safety net for the cases this cannot cover — the model yielding anyway, or the session ending — not a license to hand the wait to the user.
