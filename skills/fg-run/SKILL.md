---
name: fg-run
description: Runs a refined plan (.forge/plan.md, or a waiting plan in .forge/backlog/) through the active host's execution adapter — one unexecuted plan runs right away; several show a priority-sorted selection (with 'Run all'), then the chosen task is promoted into the active slot. Warns before re-running a plan that already ran. Use in contexts like 'forge run', 'forge execute', '계획 실행', '이거 워크플로우로 돌려줘'.
---
...
**Host contract**: read [../../core/HOST.md](../../core/HOST.md), [../../core/EXECUTION.md](../../core/EXECUTION.md), and the matching `../../hosts/<host>/execution.md` before delegating. Delegation depends on the host's `spawn_parallel` capability (and `spawn_role` for project agent cards); the matching execution adapter also owns the concrete launch, blocking-collect, and cancellation tool names and says which resumable handle or handles it returns. The shared procedure below names only those host-neutral operations. If the host is unknown or delegation is unavailable, execute serially while preserving every state transition and verification gate.
...
**1) Check the active slot.** If `.forge/plan.md` (the active slot) already exists, there is a task in progress — skip the backlog and go straight to step 4 below (only one task is active at a time). **But first look for `.forge/running.md`**: if it exists and `run.md` does **not**, the plan reached its **in-flight launch boundary** (delegation may have started, but no result was collected) — take step 4a, never the normal start, because "plan.md with no run.md" is ambiguous on its own and re-running here is exactly the duplicate execution this guard exists to prevent. If any `.forge/executed/*/STATUS.md` carries `verified: failed` while the slot is occupied, note it in one line — its recovery waits until the slot frees (see step 5). Step 4 first separates an **interrupted UAT** (verification-only resume, no re-execution) from a genuine **re-run** before doing anything destructive.
...
- **2 or more** — Present a priority-sorted selection, framed as "these are the plans fg-ask produced that haven't run yet — which one shall I execute?". **Sort every candidate first**: read each plan's `<!-- priority: high|medium|low -->` marker (see PLAN-FORMAT.md) and order them `high → medium → low`; a plan with no marker is treated as `medium`; ties within the same priority use numeric `part: N/M` order, then slug alphabetical. Show each unfinished plan as `#<task> [<priority>] <title>` (stable `task:` number, or `#—`; include the one-line goal + slice count), with "Run all (N sequential)" last. The selection UI depends on count because `AskUserQuestion` accepts at most four options:
  - **2–3 plans** → use the host's `structured_choice` capability; the plan options plus the final "Run all" option total at most four.
...
**4a) In-flight resume — `running.md` present, `run.md` absent.** The **in-flight marker** `.forge/running.md` is written by §2 immediately before launch and deleted by §4 right after `run.md` lands; if it is still there without a `run.md`, a previous turn (or session) reached the launch boundary and never collected a durable result. This is **not** a normal start and **not** a re-run — nothing may be rebuilt silently. **Read [`RECOVERY.md`](./RECOVERY.md) and follow its step 4a**: it owns the slug match, the handle collection order (running / completed / uncollectable), the rebuild confirmation, and the cancellation procedure. Rationale: ADR `260909-150614`.

**5) Failed parked-task recovery (unpark).** fg-run is the **single owner** of returning a `failed` task to execution — fg-learn and fg-done both *block* `failed`, and fg-run otherwise reaches only the active slot/backlog, never `executed/`. So if a parked `.forge/executed/<slug>/STATUS.md` carries `verified: failed`, **read [`RECOVERY.md`](./RECOVERY.md) and follow its step 5** before working on it: it owns the slot-free precondition, the unpark move, and the blocked-unpark report.

**Early-exit handoffs.** Every branch that ends the turn **without** handing off to the retro closes by rendering the handoff table (shape defined once in [`../fg-next/HANDOFF.md`](../fg-next/HANDOFF.md)), in **statement form** — never an `AskUserQuestion` (ADR-0015). The step-2 selection of a backlog task (2+ candidates) is **not** a handoff and keeps its menu. For the **0-candidates** case, nothing is in progress: `Next step` is fg-ask and `Alternative` is omitted. **For every other early exit — the verification-only resume, `verified: failed`, an in-flight wait or declined rebuild, a blocked unpark, a cancelled duplicate run — read [`RECOVERY.md`](./RECOVERY.md)'s "Early-exit handoffs" for the per-branch cells.**
...
Active slot (.forge/plan.md) present?
   │ yes ──▶ run.md present?
   │            │ no  ──▶ running.md present?  (in-flight marker, step 4a)
   │            │           │ no  ──▶ Normal start: build the workflow
   │            │           │ yes ──▶ marker slug matches plan slug?
   │            │                     │ no  ──▶ report + delete orphan marker ──▶ normal start
   │            │                     │ yes ──▶ adapter blocking-collects recorded handle(s), in order:
   │            │                              • running ──▶ wait ──▶ §3 onward
   │            │                              • completed ──▶ collect ──▶ §3 onward
   │            │                              • unknown/uncollectable/error ──▶ show age + warn it may still run elsewhere
   │            │                                                    ├─ explicit rebuild ──▶ delete marker, normal start
   │            │                                                    ├─ explicit cancel ──▶ record failed run + STATUS, delete marker, failed route
   │            │                                                    └─ decline/young marker ──▶ wait/fg-status (handoff table)
...
**Eco mode (delegated-agent efficiency).** Read `eco` from the **top-level** `.forge/config.json` (a global exemption — never the branch root; treat as `off` if the file or key is absent). If `true`, two behaviors activate for all workflow/execution subagents: **(1) model cap** — cap the subagent model at `sonnet` (e.g. `model: sonnet` on agent calls): only ever lowers, never raises; explicit user model instruction wins; **(2) Eco code-simplicity injection** — prepend the full contents of [`../fg-config/ECO.md`](../fg-config/ECO.md) to each subagent's prompt. Together these tier the loop as "strong = main session (design + grilling), efficient = delegated execution." **Beyond the subagents, when `eco` is `true` the main session running this skill also adopts ECO.md's output discipline for its own execution/reporting/handoff prose** (behavior 4 in fg-config) — this is the only carrier on the **direct-execution path** (a small task handled here without a workflow), where there are no subagents to receive the injection. Toggle via the `fg-config` utility; rationale in ADR-0014 (eco subagent model tiering). If `eco` is `false` or absent, no behavior change.
...
Once the orchestration script is ready, **get user approval first**. After approval, run it as parallel delegated work while this session keeps responding and receives progress. Use the matching host adapter's progress surface when it has one.

**Immediately before the first launch call, write the in-flight marker `.forge/running.md`** (the active slot, next to plan.md), with `workflow: pending`; as launch calls return resumable handles, replace `pending` with the complete handle list accumulated so far before launching more work or doing anything else:

```
<!-- forge-running: {slug} -->
workflow: {`pending`, then the host's resumable handle(s) when available; Claude records the workflow Task ID}
started: {epoch seconds}
session: {this session's id}
```

Writing it before launch closes the crash window between starting side effects and recording their identity. Until `run.md` is written, delegated work may outlive this turn and otherwise leave the state on disk as "plan.md only", indistinguishable from a promoted-but-unrun plan; every reporter would then say "next: fg-run" and a re-entry would rebuild the workflow (ADR `260909-150614`). The marker is what step 4a, fg-status's state machine, the session-start hook, fg-doctor (A10), fg-drop, and the sealing paths read instead.

**Then do not end the turn — use the adapter's blocking-collect operation and wait here.** Repeat collection as the adapter requires until the delegated work reports completion, then continue straight into §3 in the same turn. The marker is the safety net for the cases this cannot cover — the model yielding anyway, the session ending, or a host without a resumable handle — not a license to hand the wait to the user.
...
This is **not** an eco-only concern: record it whether or not `eco` is on. It is the **material guarantee** for anything downstream that reports per-slice results — fg-done's seal summary already instructs "result per slice" (ADR-0032), and eco's summary table renders the slice rows (`../fg-config/ECO.md`), but a later seal may happen in a **different session** where free prose is all that remains. Without these lines a reader would have to invent results, which is exactly the "instruction prose out of step with the actual contract" failure a retro already caught (`.forge/retro/2026-07-06-fg-done-seal-summary.md`). Keep the lines short — the prose below them carries the reasoning.
...
**Right after writing `run.md`, delete `.forge/running.md`** — the in-flight marker's job ends where the durable result begins; a leftover marker next to a `run.md` is what fg-doctor A10 flags. **Cancellation and abort are not exceptions that delete only the marker:** write `run.md` first with the stopped slice and observed side effects, write the companion STATUS below with `verified: failed (aborted — <reason>)`, then delete the marker and take the failed branch. Then write a companion `STATUS.md` (in the user's language) to mark an ordinary completed run as executed — it travels alongside the plan/run files as a status marker, not a second ledger (the source of truth for state is still file location). In the single-task path, write it to `.forge/STATUS.md` (the active slot, next to plan.md/run.md). Content:
...
Flow: build the workflow → approve the script → write `running.md` with `workflow: pending` → launch parallel execution → update `workflow:` with each returned resumable handle (Claude: workflow Task ID) → blocking-collect in this turn → cross-verify against the plan → record `.forge/run.md` → delete `running.md` → write `.forge/STATUS.md` (status: executed) → **run the handoff UAT and set `verified:`** (see "Verify before handing off") → only then offer fg-learn
...
Non-sealing outcome:
- **`failed (<reason>)`** — the UAT ran and the result does **not** achieve the goal. This is distinct from `pending` (which means the UAT never happened): `failed` records that verification was done and the work is broken. Write it so the state is durable and recovery is unambiguous — do not leave it as `pending` (that would look like an interrupted handoff) and do not paper over it with a `skipped` waiver. A `failed` task does not go to fg-learn or seal; route it to a fix-in-follow-up (then re-run → fresh `run.md` → re-verify) or re-grill via fg-ask. **A known-`failed` result has no waiver that seals it** — do not convert it to `skipped` to slip it through. It becomes sealable only when a fresh fix-and-re-run re-verifies it to `yes` or `n/a`. Advisory, not a hard rule: if the failure revealed a missing regression guard, consider promoting an **eval** (a persistent test) at retro time — PLAN-FORMAT.md's fix-forward eval rule binds only newly authored fix-forward plans, not this re-run of the same plan (ADR `260906-171420`). Also advisory: when the *cause* is unclear, run fg-debug before fixing; it classifies the repro as persistent or one-off, cleans diagnostic artifacts, and returns the diagnosis package to this same active task's fix-and-re-run path rather than creating a new plan or using `fg-quick`.
...
When execution finishes, **render the handoff table** — its shape is defined once in [`../fg-next/HANDOFF.md`](../fg-next/HANDOFF.md) (or `${PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/skills/fg-next/HANDOFF.md`); reference it, never restate the layout here. Render it in the user's language. Everything below is either a gate that must clear **before** you render it, or the material its cells carry.
...
**Simple mode: auto-seal instead of the retro handoff (ADR `260905-212045`).** Read `simple` from the **top-level** `.forge/config.json` (the same global-exemption read as `eco`; treat as `false` if the file or key is absent). If `true` and the UAT above recorded a **sealable** `verified:` (`yes`/`skipped`/`n/a`): record `retro: skipped (simple mode)` in STATUS (unconditional — divergence does not gate it; the same relaxation family as fg-next all / fg-loop) and invoke fg-done's deterministic seal path **in this same turn**. It is a **delegated seal** — keep it terse, never the explicit-single-seal summary chapter (ADR-0032); report the seal in one line (the eco summary table still applies when `eco` is on), then render the handoff table pointing at the next task (fg-run picks up the backlog; fg-ask when the backlog is empty). **The verification gate is untouched (ADR-0009)**: on `verified: pending` or `failed` nothing is auto-sealed — those branches run exactly as with `simple` off. The divergence cases below apply only when `simple` is `false` or absent.

**Eco: the eco summary table replaces the recap, not the handoff table.** If `eco` is `true` in the **top-level** `.forge/config.json` (the same read as §1), **replace** the what-just-happened material below with the single-task **eco summary table** — its shape is defined once in [`../fg-config/ECO.md`](../fg-config/ECO.md) (section "eco summary table"); reference it, do not restate the layout here. Nothing that material conveys may be dropped in the fold: the `verified:` value goes in the header (ADR-0009 — it is what tells the reader whether this can be sealed), the plan's Goal into `▸ Request`, and the per-slice results from `run.md`'s slice lines (§4) into `▸ Done`. The handoff table still follows it — how the two combine is HANDOFF.md's "Interaction with eco" (follow it; do not restate it here), which is what keeps the next step in the same place whether eco is on or off. Either way it stays **statement form**: no menu, no `AskUserQuestion`, no "shall I proceed?" — ADR-0015's 2026-06-15 amendment governs the tables exactly as it governed the prose. If `eco` is `false` or absent, the material below fills the handoff table unchanged.
