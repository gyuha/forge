...
**Part-plans (split tasks).** If plans carry a `<!-- part: N/M -->` marker (a big task fg-ask split into an ordered series — see PLAN-FORMAT.md), append `(part N/M)` to their label and keep them in sequence order (their `<base>-NofM` slugs already line them up under the slug tiebreak). Recommend completing them one at a time, in order, each as its own full `execute → learn → done` loop. This is a **soft order** — do not block a later part because an earlier one isn't sealed; the user may still pick any item.
