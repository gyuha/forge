...
## Fix-forward eval rule (persistent regression check)

This is the single definition of the rule (ADR `260906-171420`, eval promotion); producers reference it, never restate it.

- **Scope — the discriminator is whether a NEW plan file is being written.** The rule binds every plan authored fresh to repair a verified failure or a confirmed finding: `<!-- generated-by: fg-adversarial-review -->`, `<!-- generated-by: fg-security -->`, `<!-- generated-by: fg-debug -->`, and fg-loop's auto-generated fix-forward plans (`<!-- generated-by: fg-loop -->`). It does **not** bind an edit to a plan that has already run — that covers both fg-run's fix-and-re-run **and fg-loop's `verified: failed` in-place repair** (`<!-- repaired-by: fg-loop -->`), whose own contract is marker-only and slice-preserving, so demanding a new slice there would order it to break that contract. In those two the rule is **advisory**: add the regression check if it fits the repair, otherwise let the retro promote it (fg-learn's fourth destination). The fg-quick lane is out of scope entirely (no formal artifacts). *(Amended — the original scope included the in-place repair and paired fg-loop's generated plans with the wrong marker; both were self-contradictory, ADR `260906-171420` decision 2 as amended.)*
- **Rule: when the original failure is mechanically checkable, the plan MUST include a work slice that adds a persistent regression check** — a test in the project's own suite, or, in a repo with no test suite, an equivalent executable landing spot (e.g. an fg-doctor check in forge's own repo) — **and the Definition of Done must prove that check red → green** (fails on the pre-fix state, passes after the fix). The **red half IS the authoring-time run** the rule above demands: run the new check against the un-fixed state and record that failure as the DoD item's pre-state, so this does not exempt anyone from "run each command once at authoring time" — it names what that run must show. When the failure is not mechanically checkable (a visual UAT judgment, or a check that cannot be persisted without committing something that must not be committed — an exploit reproduction is the archetype, see fg-security), the plan carries a one-line reason **inside its `완료 정의(DoD)` section**, so a consumer (and any future fg-doctor check) finds it in one determined place — the exemption is a detectable artifact, never silence (the same discipline as the runnable-DoD "already-passing needs a reason" line and `retro: skipped (reason)`). In a repo with **no** test suite, creating the first test is itself a valid landing spot — "there is no suite" is not a reason, it is the work.
- **Enforcement: no new gate, and be honest about the gap.** Once the slice and its DoD are written into the plan, the existing verification gate (ADR-0009) enforces them like any other DoD item — an unmet DoD yields `verified: failed` and blocks sealing (and `skipped (reason)` stays an auditable waiver). But **nothing mechanically catches a fix-forward plan that omits the slice in the first place** — the gate reads the DoD that is there, not the one that should have been. That gap is accepted for now (a candidate fg-doctor check), and it is the reason the exemption must be a written line rather than an absence.
- **References.** Such a check is an **eval** in the glossary sense — a promoted check that survives sealing, unlike a DoD command, which dies with the archive. fg-security's existing "vulnerability no longer reproduces" DoD is the precedent for this rule's **red → green phrasing**, not an instance of an eval: it is a DoD command that dies with the archive, which is exactly the distinction the glossary draws.
