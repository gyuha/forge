...
## Script resolver (deterministic implementation — ADR-0022)
...
A branch's forge content is integrated into `.forge/` by **`fg-merge`** (run *after* `git merge` brings the namespaced folder into the default branch), **not** by git. fg-merge (via the deterministic `forge-merge.sh`/`.js`) moves the branch's incoming **time-based** ADRs into `.forge/adr/` as-is — an exact-ID clash takes the **next free letter** (no cascade renumber; the ADR-FORMAT.md ID scheme), rewriting that bumped ADR's cross-references — moves retros, merges CONTEXT.md terms term-by-term, folds in `done/`+`backlog/` history with one task-number remap, moves `dropped/`, and removes the branch root. The global exemptions (`config.json`, `codebase/`) are not integrated — they were never branch-local. See `skills/fg-merge/SKILL.md`.
