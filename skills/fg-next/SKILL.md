...
- **This skill cannot set `/goal` itself** — `/goal` is a harness slash command the **user** types; a skill has no tool to engage it. **So treat engaging `/goal` as the operating premise for an unattended `all` drive, not an optional aside** (the "usage pattern you may invoke" framing is exactly what users forget, so the drive stalls after one cycle): surface the paste-ready line prominently at the entry confirmation, and **re-print it whenever the drive resumes** (stateless re-issue). Be honest about the fallback in the same breath: without `/goal`, `all` mode drives one cycle and then the turn ends — most often right after a delegated fg-run/fg-done handoff *states* its next step (ADR-0015) and the model yields — which is **expected, not a failure**; re-issuing `fg-next all` resumes where it left off. (The no-arg `fg-next` is unchanged — still one-shot.)
...
- **Safety walls and the default one-shot are unchanged.** `/goal` only automates the *stateless resume* a human would otherwise type by hand; it changes none of fg-next's logic. The four halt conditions still halt, and the no-arg `fg-next` is still one-shot per task (learn→done follow-through aside, ADR-0026).
