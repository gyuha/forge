...
- **A delegated seal stays terse.** A seal reached inside a drive is a *delegated* seal — it uses fg-done's terse completion notice, **never** the explicit-single-seal summary chapter (ADR-0032). That summary is for a human's bare `/fg-done` only; in an unattended drive it would be a wall of text against the momentum this discipline exists to keep.
  - **Eco: terse becomes one row per task, not prose.** When `eco` is `true` in the **top-level** `.forge/config.json`, a drive reports each delegated seal as **one row** of the batch **eco summary table** — accumulated across the drive — instead of a per-task prose notice. The shape is defined once in [`../fg-config/ECO.md`](../fg-config/ECO.md); reference it, never restate the layout. **The summary chapter stays forbidden** — a row is not a chapter. This does not loosen the terseness rule above, it *implements* it: a row is shorter than the notice it replaces (ADR-0032's 2026-07-31 amendment). Both lanes (fg-next `all`, fg-loop) inherit this from here; neither restates it. When `eco` is `false` or absent, the terse prose notice above is unchanged.
- **`simple` changes nothing in a drive.** A drive already auto-skips retros and seals per task, so `simple: true` in the **top-level** `.forge/config.json` is a no-op here — it exists for the *conversational* fg-run path (ADR `260905-212045`). Do not double-apply it.
- **Honest limit.** Skill text cannot guarantee the model will not yield after a delegated handoff. Continue as far as the turn allows; Part 2 owns host-supported cross-turn continuation and its fallback. Neither mechanism permits crossing an unresolved wall.

## Part 2 — Cross genuine turn boundaries: forge's own Stop hook (`/goal` is the fallback)

Some boundaries end the turn no matter how forceful Part 1 is: a background Dynamic Workflow running **async** (the turn ends and a task-notification resumes it later **unless fg-run uses the host's blocking-collect to wait for it in this turn**), a workflow **script approval** (the human is physically required), or the model simply yielding despite Part 1. Crossing these unattended needs a **Stop hook** — the one harness mechanism that can prevent a turn from ending. **forge now ships its own** (ADR-0028 amended 2026-08-22), so an unattended drive needs no user action; the harness `/goal` remains as the fallback.
...
- **An unknown or uncollectable `running.md` is a wall, not a continuation boundary.** When delegated fg-run reaches its 4a case 3 because blocking-collect cannot identify or collect one or more recorded handles — whether `workflow:` is still `pending`, a same-session handle is unavailable, collection errors, or execution may still be live in another session — neither drive may confirm a re-run for the human. Delete `drive.md` and stop under the invoking lane's wall rule; do not let the Stop hook spin on a decision the orchestrator cannot make.
...
- **The hook is a host capability, not a given — check `prevent_stop`.** forge's `Stop` hook is a Claude Code hook event; whether another host dispatches `hooks/hooks.json`'s `Stop` entry is a *host* fact, recorded as the `prevent_stop` capability in `../../hosts/<host>/capabilities.json` (see [`../../core/HOST.md`](../../core/HOST.md)). **When `prevent_stop` is `false` or the host is unknown, do not write the marker and do not imply continuous execution** — the drive is turn-bounded and resumes on re-trigger (the honest fallback below). Treating an unverified host as if the hook were live is the one failure this bullet exists to prevent: the drive would promise unattended progress it cannot make, and the marker would be left behind with nothing to clear it.
...
│      └─ genuine boundary (async workflow not held by blocking-collect · script approval · yield despite Part 1) ──▶ turn would end;
