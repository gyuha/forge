...
<!--
VENDORED — this file is cloudflare/security-audit-skill's `skills/security-audit/SKILL.md`,
copied verbatim under its MIT licence (see the LICENSE beside this file).
Upstream: https://github.com/cloudflare/security-audit-skill  (vendored 2026-08-20)

ONLY the filename changed: `SKILL.md` -> `AUDIT.md`. forge auto-discovers skills at
`skills/<name>/SKILL.md`, so keeping the upstream name here would collide with forge's own
entry file, and nesting it in a subdirectory risks being mis-discovered as a second skill.
The eleven sibling files keep their upstream names and contents byte-for-byte, so an
upstream diff stays cheap. Do NOT edit the body — forge's glue lives in SKILL.md instead
(ADR `260820-215004`; the vendoring precedent is ADR `260719-224442`, whose decision 6
also exempts vendored files from the bash+node twin convention of ADR-0022).
-->
