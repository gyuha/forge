...
**The methodology is not forge's.** It is [cloudflare/security-audit-skill](https://github.com/cloudflare/security-audit-skill), vendored here under MIT (see `LICENSE`): a six-phase, multi-agent audit with attack-class playbooks, independent verification, and a machine-readable findings schema. **Read [`AUDIT.md`](./AUDIT.md) and follow it** — that file is the audit's single definition and this skill never restates or forks it (the same file-reference convention as `../fg-config/ECO.md` and `../fg-showme/VISUAL.md`). Do **not** edit `AUDIT.md` or its eleven sibling files: they are kept byte-for-byte with upstream so a future diff stays cheap, and forge's own glue lives here instead. Rationale: `.forge/adr/260820-215004-fg-security-vendored-audit.md`.
...
**Host contract**: the attack-class hunting phase fans out in parallel, so it needs the host's `spawn_parallel` capability. Read [../../core/HOST.md](../../core/HOST.md) and [../../core/EXECUTION.md](../../core/EXECUTION.md), then check `../../hosts/<host>/capabilities.json`. When `spawn_parallel` is `false` or the host is unknown, **run the attack-class playbooks serially** — this is exactly what the vendored `AUDIT.md` already instructs ("use the platform's equivalent capabilities while preserving the specified roles, parallelism, prompts"), and the severity gate, the out-of-repo artefact location, and the always-skip-in-unattended rule do not change with the host.
...
**The reason is the whole point of this skill, so do not "improve" it back into the repo.** An audit report is a list of exploitable vulnerabilities. A path **inside** the repo can never be made safe *by construction* — it is safe only if a `.gitignore` rule excludes it, and forge **never edits the user's own `.gitignore`** (the one exception is a self-ignoring `.gitignore` forge writes *inside its own state directory*, `.forge/showme/` — see ADR `260820-215004`, amended 2026-09-02; even that is a weaker guarantee than having no path at all, which is why this skill does not use it). An earlier version of this skill wrote to `.forge/security/` and asserted "no `.gitignore` change is needed"; that was true only in forge's *own* repo, and false in exactly the repos this skill is for (ADR `260820-215004`, amended 2026-08-20). A path **outside** the repo has no commit path at all.
...
- a **Definition of Done of "the vulnerability no longer reproduces"**, phrased so it can be checked *without* the exploit being written down: "re-run this finding's reproduction per the report; it no longer triggers". That is what lets fg-run's UAT reach a sealable `verified:` (ADR-0009) instead of a vague "looks fixed". A generated plan is bound by PLAN-FORMAT.md's **fix-forward eval rule** (ADR `260906-171420`) like any other `generated-by` plan — see [`../fg-run/PLAN-FORMAT.md`](../fg-run/PLAN-FORMAT.md) for the rule itself; do not restate it here. What is specific to this skill is the **exemption**: a regression test that pins a vulnerability is often the archetypal case of "mechanically checkable but not persistable", because persisting it would commit the reproduction path the paragraph below forbids ever entering the repo. When that is so, the plan carries the one-line reason in its DoD section instead of the check — and note that the "no longer reproduces" DoD above is **not** itself an eval: it dies with the archive, which is exactly the line the glossary draws.
