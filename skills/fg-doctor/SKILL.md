---
name: fg-doctor
description: A read-only integrity health check for forge — a deterministic script (forge-doctor.sh/.js) surveys the `.forge/` state contract and the persistent docs/manifests, then reports inconsistencies with severity and an actionable fix hint for each, exiting 0 clean / 1 warnings / 2 errors so it also works AI-free as a CI gate. It catches what silently rots in hand-edited Markdown: orphaned run.md, broken/missing STATUS fields, slug-pairing mismatches (plan↔STATUS↔retro), half-sealed done/, an orphaned branch root you forgot to fg-merge, manifest version drift across the 3 places, README bilingual drift, skills missing from CLAUDE.md's list, duplicate/dangling ADR IDs (both the grandfathered NNNN and the time-based scheme), and missing script twins. It writes nothing and never auto-fixes — the fix is yours to make via fg-quick/fg-ask. An on-demand utility outside the loop (the harness `init.sh` health check applied to forge itself — ADR-0019). fg-status reports "where am I", fg-doctor reports "is the state healthy". Use in contexts like 'forge doctor', '무결성 검사', '상태 점검', 'forge 진단', 'health check', '정합성 확인', 'check forge state'.
---
