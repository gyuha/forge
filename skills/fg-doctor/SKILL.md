...
9. **A9 stale or unparseable drive.md** — an unattended drive's `drive.md` marker left behind past its 30-minute bound, or one carrying no valid `started:` — **warning** either way, hint: "delete it (the Stop hook ignores it, so nothing is blocked)". It blocks nothing, but it means some drive exited without cleaning up, and the next reader deserves to know why nothing is continuing.
10. **A10 running.md in-flight marker** — fg-run's `running.md` (written at the launch boundary, immediately before launch with `workflow: pending`, then updated with a collectable host handle when available; deleted right after `run.md` is written) present with no `plan.md` — **error** (orphan; hint: delete the marker via fg-drop or rm); present alongside `run.md` — **warning** (leftover: the run finished but the marker was not deleted); carrying no valid `started:` — **warning** (unparseable: delete it and re-enter fg-run); `started:` older than 1 hour — **warning** (stale: re-enter fg-run, which collects the result or confirms before re-running). Orphan is checked first, then leftover, then the started value.
...
- **B18 fg-debug contract coherence** — in this plugin repo, the declared and uniquely enumerated counts in both HANDOFF sections (`Applies` and `Does NOT apply`) must agree; CONTEXT's handoff count must agree with the `Applies` enumeration; and the relevant fg-debug sections must retain the active-failure return, persistent-vs-one-off eval classification, every-exit cleanup, and loop-exists/root-cause-unconfirmed exit signals. Each mismatch is a **warning**. Signals are scoped to their owning sections rather than searched across the whole file, so unrelated mentions cannot satisfy a missing route. Rationale: ADR `260907-140655`.
- **B19 fg-showme confirm snippet single definition** — in this plugin repo, `skills/fg-showme/VISUAL.md` must carry **exactly one** fenced copy of the inline confirm button, and that copy must contain both `choice:'confirm:'` (the only prefix the wake filter matches) and the `dataset.sent` re-send guard — **warning**. The button is hand-written per screen and `confirm` appears nowhere in the vendored client, so the doc snippet is the single thing that keeps every screen's button correct; two copies drifted once (retro `260805-063357` learning 3), which is why a second copy fails rather than merely warns about the count.
- **B20 capability key ↔ Host contract coherence** — in this plugin repo, every `skills/*/SKILL.md` that names a capability key from `core/HOST.md`'s table **in backticks** must carry a `**Host contract**` paragraph, and every `**Host contract**` paragraph must name at least one key; a variant label such as `**Host adapter**` is reported as a label problem only — **warning**. A backticked key name is the *declaration* signal, so a skill that merely mentions a capability in passing writes it without backticks (this bullet does exactly that: structured_choice, prevent_stop, event_wake) — the alternative, exempting the file that documents the check, is the blind spot B17 refused. The vocabulary is derived from `core/HOST.md`, never hardcoded, so a new key is covered the moment the table gains a row. This is the other half of HOST.md's "a skill names a capability and looks it up mechanically": the dependency must also be *declared* where a reader looks for it (retro `260908-210216` learning 1 — fg-showme had named `Monitor` without any declaration).
