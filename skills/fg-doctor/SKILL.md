...
**Ignore `.forge/dropped/`.** It holds incomplete work that fg-drop abandoned (hard-delete's archive alternative — ADR-0021); its contents are deliberately out of the active state contract. Do **not** scan it or flag its files (a dropped `STATUS.md` still reading `status: executed`, a dropped `plan.md`, etc.) as orphans or half-sealed — none of group A's checks apply to it.
...
7. **Stale ask.md marker.** `.forge/ask.md` marks an in-progress fg-ask grilling session (see its SKILL.md); fg-ask deletes it once the plan reaches the backlog. If `ask.md` exists and its file mtime is more than 1 day old, the grilling session behind it was likely abandoned or interrupted — **warning**, fix hint: "finish grilling with fg-ask (it will overwrite ask.md), or discard it via fg-drop."

## Check group B — docs & manifest integrity (repo root, git-tracked)

8. **Manifest version sync.** `.claude-plugin/plugin.json` `version` == `marketplace.json` `metadata.version` == `marketplace.json` `plugins[0].version`. Any mismatch — **error** (a broken release).
9. **Manifest JSON validity.** Both manifests parse as JSON (`node -e "...JSON.parse..."`). Parse failure — **error** (install breaks).
10. **Skill auto-discovery.** Every `skills/*/SKILL.md` has frontmatter `name:` (`awk '/^name:/'` over each). A missing `name` means the skill is not auto-discovered — **error**.
11. **Skill-count consistency.** The number of `skills/*/SKILL.md` matches the count word(s) in the manifest descriptions (e.g. "Eighteen … Fourteen more sit outside the loop"). A drift — **warning**.
12. **CLAUDE.md skill-list completeness.** Every skill under `skills/` is mentioned in CLAUDE.md (the loop description or the "루프 밖 스킬 / outside-the-loop" list). A skill present on disk but absent from CLAUDE.md is a violation — **warning** (this is the check that catches the fg-statusline omission).
13. **README bilingual sync.** `README.md` and `README.ko.md` have matching skill rows / list entries (compare the skill table row counts and the skill names listed). A divergence — **warning**.
14. **ADR integrity.** ADR numbers in `.forge/adr/*.md` are contiguous, or any gap is explained by `.forge/adr/retired/` (a retired number is never reused — ADR-0012). Cross-references (`ADR-NNNN`) in active ADRs point to a number that exists (active or retired). A gap with no retired entry, or a dangling reference — **warning**. (On a non-default branch, evaluate this against the combined overlay ADR set — top-level + branch root — per the Forge-root note above, not the branch root alone; otherwise a branch ADR referencing a top-level one falsely reads as dangling.)
15. **Script twin parity (ADR-0022).** Every `scripts/*.sh` operational script must have a matching `scripts/<name>.js` node twin, and vice versa (dual-dispatch convention — bash primary, node fallback). A `.sh` with no `.js` twin (or a `.js` with no `.sh`) is a violation — **warning** (the missing platform will silently fall through). This is the **static** half of the parity guard; semantic equality is verified by the `scripts/*.parity.test.sh` tests (run those for the real check). Exempt: `*.test.sh` / `*.parity.test.sh` fixtures and `*-wrapper.sh` launchers, which are not dual-dispatch ops — match the base operational scripts only.
