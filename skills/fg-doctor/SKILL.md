...
3. **Slug pairing.** The plan's first-line `<!-- forge-slug: <slug> -->` must match its `STATUS.md` `slug:`, and a retro (when one exists) must be named `retro/*-<slug>.md`. A mismatch across plan↔STATUS↔retro is a violation — **error** (it breaks fg-learn/fg-done pairing).
