...
DRIVE one task loop at a time:
   select next member ─▶ invoke fg-run with its task#/slug (fg-run promotes) ─▶ run ─▶ UAT
      │
      ├─ verified: sealable ─▶ auto-skip retro ─▶ seal ─▶ run stop-condition checks (below)
      │
...
- **Wall** → `Just did` = the wall by its precise `wall:` cause (`no-progress (C1)` / `cap-exhausted` / `unverifiable-uat (C2)` / `fork (<reason>)` / `tension (Cx↔Cy)` / `safety (<class>)`) with round N/cap — **never a bare "halted"**; `Next step` = the human resolves the wall, then re-triggering `fg-loop` resumes (stateless, `loop.md` kept); `How to start` = `/forge:fg-loop`; `Alternative` = re-grilling via `fg-ask` or widening the scope/cap, where that is the real route out.
