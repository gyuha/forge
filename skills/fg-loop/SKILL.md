...
**Do not end the turn between task loops.** Sealing one task is not a stopping point: after each seal, immediately run the stop-condition checks and continue — to the next member task, or into a replan round (§3). The only legitimate ends of a turn are **goal-met** (checks all pass) and **a wall** (§4); pausing anywhere else is the "one cycle then stall" failure this drive exists to prevent (the `/goal` pairing in §1 covers the pauses that are genuinely unavoidable, like a script approval).
...
**Tension wall — fix-forwards trading one check for another (ADR-0016 2026-06-25).** When the post-seal check run records a **regression** (a previously-passing check flipped to `fail` — §2), the in-scope fix-forward for one check broke another. **Lenient + one retry**: on the **first** regression, generate the next fix-forward with a **hard constraint to satisfy BOTH** the target check and the regressed check (fed forward alongside Reflexion, not instead of it). If it regresses **again** — the same check's `regressed: ×N` reaches `×2`, **or** two distinct checks have each regressed at least once (a ping-pong) — the checks are in genuine tension that in-scope fix-forwards can't reconcile: halt with `wall: tension (Cx↔Cy)`, report the conflicting pair and their `last-evidence`, and hand the tradeoff to the human. This fires **before** cap exhaustion, so the human reads "C1↔C2 conflict" instead of a blunt "cap-exhausted". Detection is **mechanical** (the ledger's pass→fail flip) — unlike the safety gate below, no judgment is involved.
...
A turn boundary mid-drive — most often an fg-run workflow script approval, **or simply the model yielding the turn after a delegated fg-run/fg-done handoff states its next step (ADR-0015)** — ends the turn; to run unattended across boundaries, pair with the harness `/goal`, exactly as documented in `../fg-next/SKILL.md` ("Unattended to completion — pairing with /goal"). This pairing is the **operating premise** of an unattended drive, not an optional add-on — **a skill cannot set `/goal` itself**, so §1 presents it prominently at drive entry and **re-prints it at every resume** (re-derived from this loop's walls). Without it engaged, the drive falls back to one-cycle-then-pause — expected, not broken; re-trigger `forge loop` to resume. Phrase the condition as "stop only when the stop-condition checks all pass, OR a wall is hit" — never "until the checks pass" alone, which would force the drive past the safety walls. **Enumerate the stop-allowed points from this loop's walls, not fg-next's**: goal-met (all checks pass) · unverifiable UAT (`pending`) · genuine fork / out-of-scope fix · replan cap exhausted · no progress ×2 · tension (regression ping-pong) · safety (irreversible action class) · a workflow script approval (human input is physically required). Do **not** list `verified: failed` as a stop point — in fg-loop it is the automated fix-forward case (§3), and a `/goal` that halts on it would undo exactly the wall-relaxation this skill exists for. Recommended paste-ready shape (render in the user's language):
